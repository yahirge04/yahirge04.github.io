...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const rehearsedCounter = document.getElementById('rehearsed-counter');
    ...
    const renderSongs = () => {
        ...
        if (songs.length === 0) {
            if (rehearsedCounter) {
                rehearsedCounter.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-right:4px"><polyline points="20 6 9 17 4 12"></polyline></svg> Ensayadas: 0 / 0`;
            }
            songListContainer.innerHTML = `
        ...
        const rehearsedCount = songs.filter(s => s.rehearsed).length;
        if (rehearsedCounter) {
            rehearsedCounter.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-right:4px"><polyline points="20 6 9 17 4 12"></polyline></svg> Ensayadas: ${rehearsedCount} / ${songs.length}`;
        }
        ...
        songs.forEach((song, index) => {
            ...
            colKey.className = 'col col-key';
            colKey.textContent = song.key || '-';
    ...
    songForm.addEventListener('submit', (e) => {
        ...
        const key = songKeyInput ? songKeyInput.value.trim() : '';
        ...
        exportExcelBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                worksheet.columns = [
                    ...
                    { header: 'Canción', key: 'name', width: 40 },
                    { header: 'Tonalidad', key: 'key', width: 15 },
                ...
                songs.forEach((song, index) => {
                    ...
                    // Estilo a la celda de la tonalidad
            ...
            try {
                ...
                reader.onload = async (event) => {
                    ...
                    worksheet.eachRow((row, rowNumber) => {
                        ...
                        // Soporte para archivos de Excel exportados sin la columna Nº o antiguos
                        if (val2 === 'YA ENSAYADA' || val2 === 'NO ENSAYADA') {
                            nameCell = val1;
                            keyCell = '';
                            statusCell = val2;
                        } else if (val3 === 'YA ENSAYADA' || val3 === 'NO ENSAYADA') {
                            // Validar si el encabezado especifica "Tonalidad"
                            if (worksheet.getRow(1).getCell(3).value !== 'Tonalidad') {
                                nameCell = val2;
                                keyCell = '';
                                statusCell = val3; // Era formato antiguo
                            } else {
                                nameCell = val1;
                                keyCell = val2;
                                statusCell = val3;
                        ...
                        if (nameCell) {
                            importedSongs.push({
                                ...
                                key: keyCell ? keyCell.toString().trim() : '',
